...
function wp_sugar_free_widget_fields() {
	...
	$config_field = get_option ( "smack_wp_sugar_widget_free_field_settings" );

	if ( isset ( $_POST ['widget_field_posted'] ) ) {
		$config_field ['widgetfieldlist'] = array ();	
		if (isset ( $_POST ['no_of_vt_fields'] )) {
			...
			$config_field ['widgetfieldlist'] = $fieldArr;
		}
		update_option ( 'smack_wp_sugar_widget_free_field_settings', $config_field );
	}
/*
...
<div class="sugar-free-messageBox" id="message-box" style="display:none;" ><b>Successfully Saved!</b></div>
	<form id="smack_wp_sugar_free_field_form"
		...
		if (! is_array ( $config_field ['widgetfieldlist'] )) {
			$config_field ['widgetfieldlist'] = array ();
		}
		...
		if (! empty ( $allowedFields )) {
                ...
                <?php } 
                        $content .= '<div style="width:20%;float:left;"><h3 class="title">Field settings</h3></div><br/>
		...
		<input type="button" class="button-secondary submit-add-to-menu"
			...
			onclick="wpsugarfreeupgradetopro()" /><br/>
<div style="width:80%;"><p>( Please use the short code <b> [sugarcrm_webtolead_WG]</b> in page or post )</p></div>
...
$imagepath = $contentUrl . '/plugins/wp-sugar-free/images/';
			...
			elseif(in_array ( $field['fieldid'], $config_field ['widgetfieldlist'] )) 
			{
		...
		<p>Please use the short code <b> [sugarcrm_webtolead_WG]</b> in page or post</p>';
		$content .= '<input type="hidden" name="posted" value="posted" />
                <br/>
                <input type="button" class="button-secondary submit-add-to-menu"
                        name="sync_crm_fields" value="Fetch CRM Fields"
                        onclick="wpsugarfreeupgradetopro()" />
                ...
                <input type="button" class="button-secondary submit-add-to-menu"
                        ...
                        name="save_display_name" id="save_display_name" value="Save Labels"
                        onclick="wpsugarfreeupgradetopro()" />';
		$content .='<input type="hidden" name="widget_field_posted"
			value="posted" />
